...
def test_file_not_added_by_default(values_key, filename, helm, chart_path):
    values = {}
...
def test_file_is_not_mounted_by_default(values_key, filename, helm, chart_path):
    values = {}
...
def test_file_is_added_when_configured(values_key, filename, helm, chart_path):
    values = safe_load(f"""
        portalFrontend:
          branding:
...
def test_file_is_mounted_when_configured(values_key, filename, helm, chart_path):
    values = safe_load(f"""
        portalFrontend:
          branding:
            {values_key}: stub_value
