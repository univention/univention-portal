...
async def test_prune_notifications_after_expiry(test_db):
    ...
    assert not any(notification.title == "short-lived notification" for notification in notifications)

    assert len(notifications) == 2

    # the longer-lived notification should still be there
    assert any(notification.title == "long-lived notification" for notification in notifications)
    # the non-expiring notification should still be there
    assert any(notification.title == "non-expiring notification" for notification in notifications)
...
async def test_pruner_waits_for_expiretime(test_db):
    ...
    notifications = service.get_notifications(query)
    assert (
        any(n.title == "short-lived notification" for n in notifications)
    ...
    assert any(n.title == "long-lived notification" for n in remaining) and any(n.title == "non-expiring notification" for n in remaining)
...
async def test_pruner_no_expiry(test_db):
    ...
    notifications = service.get_notifications(query)
    assert len(notifications) == 1 and all(n.expireTime is None for n in notifications)
    ...
    remaining = service.get_notifications(query)
    assert len(remaining) == 1 and remaining[0].title == "non-expiring notification"
...
async def test_pruner_soon_expiring(test_db):
    ...
    notifications = service.get_notifications(query)
    assert len(notifications) == 2 and any(n.title == "short-lived notification" for n in notifications) and any(n.title == "non-expiring notification" for n in notifications)
