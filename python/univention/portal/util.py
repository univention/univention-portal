...
def _extend_end_day_to_midnight_if_necessary(
    end_iso_datetime_str: str, range_end: datetime
):
    """
    ...
    if end_iso_datetime_str and len(end_iso_datetime_str) <= len(
        "YYYY-MM-DD"
...
def is_current_time_between(
    start_iso_datetime_str: str, end_iso_datetime_str: str
) -> bool:
    """
    ...
    range_start = _sanitize_and_parse_iso_datetime_str(
        start_iso_datetime_str, datetime(MINYEAR, 1, 1)
    )
    range_end = _sanitize_and_parse_iso_datetime_str(
        end_iso_datetime_str, datetime(MAXYEAR, 12, 31)
    )
    range_end = _extend_end_day_to_midnight_if_necessary(
        end_iso_datetime_str, range_end
    )
    if range_start <= range_end:
        return range_start <= now <= range_end
    else:
        get_logger("util").warning(
            "given time boundaries not in chronological order"
        )
        return True
...
def log_url_safe(url):
    ...
    if parts.password:
        ...
        url = urlunsplit(
            (
                parts.scheme, new_netloc, parts.path, parts.query,
                parts.fragment
            )
        )
    return url
