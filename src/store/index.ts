...
export const store = createStore<RootState>({
  ...
  actions: {
    loadPortal: ({ dispatch }) => new Promise((resolve, reject) => {
      ...
      axios.all(portalPromises).then(axios.spread((metaResponse, portalResponse) => {
        const [meta, portal] = [metaResponse.data, portalResponse.data];
        dispatch('metaData/setMeta', meta);
        dispatch('menu/setMenu', portal);
