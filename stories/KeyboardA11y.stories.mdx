...
Drag'n'Drop is an excellent example of a user interaction / input method that is exclusively developed for mouse users. It is very convenient because the user intuitively knows how to use the mouse input to sort things, move by dragging, or resize elements. As it is more convenient for the mouse user it is less convenient for the keyboard user. (jkeiser: Does *every* feature has to have a keyboard alternative or only key features? Also some features may be visual only e.g. resizing columns in a grid via dragging with mouse)There has to be an alternative way to achieve the same result by keyboard input only.
